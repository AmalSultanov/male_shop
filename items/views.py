...
class ItemListView(ListView):
    ...
    def get_queryset(self):
        ...
        if price:
            ...
            qs = qs.filter(real_price__gte=price_from,
                           real_price__lte=price_to)
...
class ItemDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['related_goods'] = self.object.category.items.exclude(
            pk=self.object.pk).order_by('-pk')[:4]
        return context
